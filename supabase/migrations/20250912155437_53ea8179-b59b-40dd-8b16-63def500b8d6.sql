...
CHECK (mobile_number IS NULL OR mobile_number ~ '^[0-9+\-\s\(\)]+$');

ALTER TABLE public.customers 
ADD CONSTRAINT check_customer_mobile_format 
CHECK (customer_mobile IS NULL OR customer_mobile ~ '^[0-9+\-\s\(\)]+$');

-- 7. Create enhanced RLS policies with better audit logging
DROP POLICY IF EXISTS "Super admins can view all profiles" ON public.profiles;
CREATE POLICY "Super admins can view all profiles with audit" 
ON public.profiles 
FOR SELECT 
USING (
  (auth.uid() = user_id) OR 
  (has_role(auth.uid(), 'super_admin'::app_role) AND audit_admin_access('profiles', id, 'ADMIN_VIEW_PROFILE'))
);

DROP POLICY IF EXISTS "Super admins can view customers with audit logging" ON public.customers;
CREATE POLICY "Super admins can view customers with enhanced audit" 
ON public.customers 
FOR SELECT 
USING (
